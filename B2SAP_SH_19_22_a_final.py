...
def DiaLookUpTable1(Composition, B1, B2, B3, B4, B5, B6, B7, B8, B9, B10, B11, B12, B13, B14, B15):
    ...
    elif Composition == 'Smoky_1.41+0.02j':
        dp_in = df1[(df1.Composition == 'Smoky_1.41+0.02j') & (df1.Binning == 'Manual')]
        
    elif Composition == 'Smoky_1.41+0.02j Auto':
        dp_in = df1[(df1.Composition == 'Smoky_1.41+0.02j') & (df1.Binning == 'Automatic')]

    elif Composition == 'Smoky_1.48+0.02j':
        dp_in = df1[(df1.Composition == 'Smoky_1.48+0.02j') & (df1.Binning == 'Manual')]
        
    elif Composition == 'Smoky_1.48+0.02j Auto':
        dp_in = df1[(df1.Composition == 'Smoky_1.48+0.02j') & (df1.Binning == 'Automatic')]
...
def plotIOR(df):#take one profile and calculate extinction with two different indices of refraction
    ...
    #Signal B
    dp_in_B = df1[(df1.Composition == 'Smoky_1.41+0.02j') & (df1.Binning == 'Manual')]
    ...
    #Signal C
    dp_in_C = df1[(df1.Composition == 'Smoky_1.48+0.02j') & (df1.Binning == 'Manual')] 
    dp_C =  np.array(dp_in_C.Diameter*1000).tolist()
    signal_C =  np.array(dp_in_C.Signal).tolist()
    
    sig_C_f = np.ones(15, dtype=float)
    dp_C_f = np.ones(15, dtype=float)
    
    for num in range (0,15): 
        #print (num)
        dp_C_f[num] = (dp_C[num] + dp_C[num+1])/2
        sig_C_f[num] = (signal_C[num] + signal_C[num+1])/2
   
    dp_C = dp_C_f
    signal_C = sig_C_f
    
    #Signal D
    dp_in_D = df1[(df1.Composition == 'Sulfate') & (df1.Binning == 'Manual')] 
    dp_D =  np.array(dp_in_D.Diameter*1000).tolist()
    signal_D =  np.array(dp_in_D.Signal).tolist()
    
    sig_D_f = np.ones(15, dtype=float)
    dp_D_f = np.ones(15, dtype=float)
    
    for num in range (0,15): 
        #print (num)
        dp_D_f[num] = (dp_D[num] + dp_D[num+1])/2
        sig_D_f[num] = (signal_D[num] + signal_D[num+1])/2
    ...
    if plots == 1:
        ...
        plt.plot(signal_B, dp_B, 'rs-')
        plt.plot(signal_C, dp_C, 'ro-')
        plt.plot(signal_D, dp_D, 'bs-')
        plt.plot(signal_A, dp_A, 'ks-')
        plt.xlabel('POPS signal (ADC)', fontsize=17)
        plt.ylabel('Mean POPS Bin Diameter (nm)', fontsize=17)
        plt.legend(['Smoke Alt IR = 1.41 + 0.02j', 'Smoke Alt IR = 1.48 + 0.02j', 'Sulfate IR = 1.45 + 0.0j', 'Smoke IR = 1.54 + 0.018j'])
        plt.yscale('log')
        plt.xscale('log')
        ax.text(10000, 3000, 'a.', size=14, color='black')
        ax.grid()
        ...
        df_select['Composition_B'] = 'Smoky_1.41+0.02j'
        df_select['Composition_C'] = 'Smoky_1.48+0.02j'
        df_select['Composition_D'] = 'Sulfate'
        ...
        df_select[['Sim_SA_C', 'Sim_SA_D']] = df_select.apply(lambda x: pd.Series(CalcSA(x['Composition_C'], x['Composition_D'], x['B1'], x['B2'], x['B3'], x['B4'], x['B5'], x['B6'], x['B7'], x['B8'], x['B9'], x['B10'], x['B11'], x['B12'], x['B13'], x['B14'], x['B15'])), axis=1)
        ...
        df_select[['Sim Extinction_532 C', 'Sim Ambient Extinction_532 C']] = df_select.apply(lambda x: pd.Series(VPsca(x['Wavelength1'], x['Composition_C'], x['RH_FPHc'], x['Air Temp K'], x['H2SO4_%wt_pops'], x['Density_pops'], x['H2SO4_%wt_amb'], x['Density_amb'],x['B1'], x['B2'], x['B3'], x['B4'], x['B5'], x['B6'], x['B7'], x['B8'], x['B9'], x['B10'], x['B11'], x['B12'], x['B13'], x['B14'], x['B15'])), axis=1)
        df_select[['Sim Extinction_532 D', 'Sim Ambient Extinction_532 D']] = df_select.apply(lambda x: pd.Series(VPsca(x['Wavelength1'], x['Composition_D'], x['RH_FPHc'], x['Air Temp K'], x['H2SO4_%wt_pops'], x['Density_pops'], x['H2SO4_%wt_amb'], x['Density_amb'], x['B1'], x['B2'], x['B3'], x['B4'], x['B5'], x['B6'], x['B7'], x['B8'], x['B9'], x['B10'], x['B11'], x['B12'], x['B13'], x['B14'], x['B15'])), axis=1)
        
        
        df_select = df_select.filter(['Launch', 'Altitude (km)', 'Composition_A', 'Composition_B', 'Sim_SA_A', 'Sim_SA_B', 'Sim_SA_C', 'Sim_SA_D', 'Sim Extinction_532 A', 'Sim Ambient Extinction_532 A', 'Sim Extinction_532 B', 'Sim Ambient Extinction_532 B', 'Sim Extinction_532 C', 'Sim Extinction_532 D'])

        x_ones = np.linspace(0.0, 25.0, num=1000)
        y_ones = np.linspace(0.0, 25.0, num=1000)
        #Figure 1b
        fig, ax = plt.subplots(1, 1, figsize=(8, 4))

        plt.plot(df_select['Sim_SA_B'].rolling(5, center=True).mean(), df_select['Altitude (km)'], 'r-')
        plt.plot(df_select['Sim_SA_D'].rolling(5, center=True).mean(), df_select['Altitude (km)'], 'b-')
        plt.plot(df_select['Sim_SA_A'].rolling(5, center=True).mean(), df_select['Altitude (km)'], 'k-')
        plt.xlabel('Surface Area (\u03bc$\mathregular{m^{2}}$ $\mathregular{cm^{-3}}$)', fontsize=15)
        plt.ylabel('Altitude (km)', fontsize=15)
        plt.legend(['Smoke Alt IR = 1.41 + 0.02j', 'Sulfate IR = 1.45 + 0.0j', 'Smoke IR = 1.54 + 0.018j'])
        plt.ylim([10.0, 28.0])
        plt.xlim([-1, 25])
        plt.title(Title)
        plt.rc('xtick', labelsize=14) 
        plt.rc('ytick', labelsize=14) 
        ax.text(9, 26, 'b.', size=14, color='black')
        
        
        [slope_a, intercept_a, r_value_a, p_value_a, std_err_a] = stats.linregress(df_select['Sim_SA_A'], df_select['Sim_SA_B'])
        [slope_c, intercept_c, r_value_c, p_value_c, std_err_c] = stats.linregress(df_select['Sim_SA_A'], df_select['Sim_SA_D'])
        fig = plt.figure()
        plt.plot(df_select['Sim_SA_A'], df_select['Sim_SA_B'], 'ko')
        plt.plot(df_select['Sim_SA_A'], df_select['Sim_SA_D'], 'k^')
        line_x = np.linspace(0.5, 20, num=100)
        plt.plot(line_x, line_x*slope_a+intercept_a, 'r-')
        plt.plot(line_x, line_x*slope_c+intercept_c, 'b-')
        plt.plot(x_ones, y_ones, 'k--')
        plt.xlabel('IR = 1.54 + 0.018j Surface Area (\u03bc$\mathregular{m^{2}}$ $\mathregular{cm^{-3}}$)')
        plt.ylabel('Other IR Surface Area (\u03bc$\mathregular{m^{2}}$ $\mathregular{cm^{-3}}$)')
        plt.legend(['m1 = ' + str(np.round(slope_a,2)) +'; m2 = ' + str(np.round(slope_c,2))]) #['y = 1.38x + -0.41'])
        plt.title(Title)
        plt.xlim([0, 20])
        plt.ylim([0, 25])
        ...
        #Figure 1c
        fig, ax = plt.subplots(1, 1, figsize=(8, 4))
        plt.plot(df_select['Sim Extinction_532 B'].rolling(5, center=True).mean(), df_select['Altitude (km)'], 'r-')
        plt.plot(df_select['Sim Extinction_532 D'].rolling(5, center=True).mean(), df_select['Altitude (km)'], 'b-')
        plt.plot(df_select['Sim Extinction_532 A'].rolling(5, center=True).mean(), df_select['Altitude (km)'], 'k-')
        plt.xlabel(' \u03B5 ($\mathregular{km^{-1}}$ \u03BB = 532)', fontsize=15)
        plt.ylabel('Altitude (km)', fontsize=15)
        plt.legend(['Smoke Alt IR = 1.41 + 0.02j', 'Sulfate IR = 1.45 + 0.0j', 'Smoke IR = 1.54 + 0.018j'])
        plt.ylim([10.0, 28.0])
        ...
        plt.rc('xtick', labelsize=14) 
        plt.rc('ytick', labelsize=14) 
        ax.text(0.006, 26, 'c.', size=14, color='black')

        
        [slope_b, intercept_b, r_value_b, p_value_b, std_err_b] = stats.linregress(df_select['Sim Extinction_532 A'], df_select['Sim Extinction_532 B'])
        [slope_d, intercept_d, r_value_d, p_value_d, std_err_d] = stats.linregress(df_select['Sim Extinction_532 A'], df_select['Sim Extinction_532 D'])
        fig = plt.figure()
        plt.plot(df_select['Sim Extinction_532 A'], df_select['Sim Extinction_532 B'], 'ko')
        plt.plot(df_select['Sim Extinction_532 A'], df_select['Sim Extinction_532 D'], 'k^')
        line_x = np.linspace(0.00001, 0.010, num=100)
        plt.plot(line_x, line_x*slope_b+intercept_b, 'r-')
        plt.plot(line_x, line_x*slope_d+intercept_d, 'b-')
        plt.plot(x_ones*0.01, y_ones*0.01, 'k--')
        plt.xlabel('IR = 1.54 + 0.018j \u03B5 ($\mathregular{km^{-1}}$ \u03BB = 532)')
        plt.ylabel('Other IR \u03B5 ($\mathregular{km^{-1}}$ \u03BB = 532)')
        plt.legend(['m1 = ' + str(np.round(slope_b,2)) +'; m2 = ' + str(np.round(slope_d,2))]) #['y = 1.20x + -1.6E-5'])
        plt.title(Title)
...
def DnDlogDp(Launch, Composition, B1, B2, B3, B4, B5, B6, B7, B8, B9, B10, B11, B12, B13, B14, B15):
    ...
    if Composition == 'Smoky':
        ...
        dp_in = dfIOR[(dfIOR.Composition == 'Smoky_1.41+0.02j') & (dfIOR.Binning == 'Manual')]
        dLogDp = np.array(dp_in.dLogDp)
        ...
        dp_in = dfIOR[(dfIOR.Composition == 'Smoky_1.41+0.02j') & (dfIOR.Binning == 'Automatic')]
        dLogDp = np.array(dp_in.dLogDp)
...
def Plumeb(Launch, Flight, Altitude):
    if (Launch == '2020-01-27'):
  
        if ((Altitude >= 15.0) and (Altitude <= 17.5)):
            composition = 'Smoky_1.41+0.02j Auto'
        else:
            composition = 'Sulfate Auto'
    elif (Launch == '2019-04-30'):
        composition = 'Sulfate Auto'
...
def calcTsUnc(Launch, Property, VertIntTot):
    ...
    if (Property == 'sAOD'):
        if (Launch == '2019-04-30'):
            POPS_Unc = VertIntTot * 0.67
        elif (Launch == '2019-09-03'):
                POPS_Unc = VertIntTot * 0.55
        elif (Launch == '2020-01-27'):
                POPS_Unc = VertIntTot * 0.41        
        elif (Launch == '2020-06-19'):
                POPS_Unc = VertIntTot * 0.67  
        elif (Launch == '2020-07-23'):
                POPS_Unc = VertIntTot * 0.41  
        elif (Launch == '2020-11-17'):
                POPS_Unc = VertIntTot * 0.62 
        elif (Launch == '2021-08-31'):
                POPS_Unc = VertIntTot * 0.49
        elif (Launch == '2021-11-03'):
                POPS_Unc = VertIntTot * 0.66
        elif (Launch == '2020-01-24'):
                POPS_Unc = VertIntTot * 0.81  
    else:
...
def SDed(Concentration, Source, Type, Theta):
    Concentration_ed = Concentration
    
    if Source == 'POPS_IR=1.41+0.02j':
        if Type == 'Volcanic':
            Concentration_ed = np.NaN
        elif Type == 'Approaching Baseline':
            Concentration_ed = np.NaN
        elif ((Type == 'BB') & (Theta == 510)):
            Concentration_ed = np.NaN
            print('test case')
    
    return Concentration_ed
...
CARMA=pd.read_csv(file, delimiter = ',', header = 0,  dtype=None, low_memory=False) 
file = '/Users/asher/Documents/PapersInProgress/JGR_ANYSO/Submission/POPSDataForPublication/POPS/POPS_table.csv'
df_p = pd.read_csv(file, delimiter = ',', header = 20,  dtype=None, low_memory=False)
...
df_AscDesc['CompositionB'] = df_AscDesc.apply(lambda x: Plumeb(x['Launch'], x['Flight'], x['Altitude (km)']), axis=1)
...
df_AscDesc['dNdLogDpB'] = df_AscDesc.apply(lambda x: DnDlogDp(x['Launch'], x['CompositionB'], x['B1'], x['B2'], x['B3'], x['B4'], x['B5'], x['B6'], x['B7'], x['B8'], x['B9'], x['B10'], x['B11'], x['B12'], x['B13'], x['B14'], x['B15']), axis=1)
df_AscDesc['DpB'] = df_AscDesc.apply(lambda x: DiaLookUpTable1(x['CompositionB'], x['B1'], x['B2'], x['B3'], x['B4'], x['B5'], x['B6'], x['B7'], x['B8'], x['B9'], x['B10'], x['B11'], x['B12'], x['B13'], x['B14'], x['B15']), axis=1)
...
df_POPSsizedist = df_AscDesc.filter(['Launch', 'Altitude (km)', 'Flight', 'Dp', 'dNdLogDp','DpB', 'dNdLogDpB', 'Theta [K]'])
#import lidar data
...
df2['Measured Bratio_532'] = df2['Measured Extinction_532']
df2['Ambient Bratio_532'] = df2['Ambient Extinction_532']
...
df_extinction.to_csv('/Users/asher/Desktop/LauderExtinction_UTLS_IOR_1.54_0.03j.csv', mode='w', float_format='%g')
...
SD_colnames = ['100.0', '131.2', '172.2', '226.0', '296.0', '388.0', '510.0', '670.0', '878.0', '1152.0',
                  '1512.0', '1984.0', '2603.22']
...
CARMA_ac_test['Aer Conc calc. SD >= 115 nm'] = CARMA_ac_test[['131.2', '172.2', '226.0', '296.0', '388.0', '510.0', '670.0', '878.0', '1152.0',
                  '1512.0', '1984.0', '2603.22']].sum(axis=1)

CARMA_ac_test['Aer Conc calc. SD >= 198 nm'] = CARMA_ac_test[[ '172.2', '226.0', '296.0', '388.0', '510.0', '670.0', '878.0', '1152.0',
                  '1512.0', '1984.0', '2603.22']].sum(axis=1)

CARMA_ac_test['Extinction calc. SD'] =  CARMA_ac_test.apply(lambda x: VPscaCARMA(x['Launch'], x['GeoPotential Height [km]'], x['100.0'], x['131.2'], x['172.2'], x['226.0'], x['296.0'], x['388.0'], x['510.0'], x['670.0'], x['878.0'], x['1152.0'], x['1512.0'], x['1984.0'], x['2603.22'], x['3415.62'], x['4481.56'], x['5880.16'], x['7715.22'], x['10123.0'], x['13282.1'], x['17427.1']), axis=1)



CARMA_ac_test = CARMA_ac_test.filter(['Launch', 'Potential T [K]', 'GeoPotential Height [km]', 'Level Height [km]', 'Aerosol Concentration [#/cm3]','Number Concentration Column [# m^-2]', 'Aer Conc calc. SD >= 198 nm', 'Aer Conc calc. SD >= 115 nm', 'Extinction calc. SD', 'sAOD'], axis=1)
...
CARMA = CARMA.filter(['Launch', 'Theta', '100.0', '131.2', '172.2', '226.0', '296.0', '388.0', '510.0', '670.0', '878.0', '1152.0',
                  '1512.0', '1984.0', '2603.22'])
...
df_POPSsizedist[['dNdLogDp1B','dNdLogDp2B', 'dNdLogDp3B', 'dNdLogDp4B', 'dNdLogDp5B', 'dNdLogDp6B', 'dNdLogDp7B', 'dNdLogDp8B', 'dNdLogDp9B', 'dNdLogDp10B', 'dNdLogDp11B', 'dNdLogDp12B', 'dNdLogDp13B', 'dNdLogDp14B', 'dNdLogDp15B']] = pd.DataFrame(df_POPSsizedist.dNdLogDpB.tolist(), index= df_POPSsizedist.index)
df_POPSsizedist[['Dp1B','Dp2B', 'Dp3B', 'Dp4B', 'Dp5B', 'Dp6B', 'Dp7B', 'Dp8B', 'Dp9B', 'Dp10B', 'Dp11B', 'Dp12B', 'Dp13B', 'Dp14B', 'Dp15B']] = pd.DataFrame(df_POPSsizedist.DpB.tolist(), index= df_POPSsizedist.index)
...
POPSsd_m_concB = POPSsd_m.filter(['Launch', 'Theta', 'Altitude (km)', 'Theta [K]', 'dNdLogDp1B',
       'dNdLogDp2B', 'dNdLogDp3B', 'dNdLogDp4B', 'dNdLogDp5B', 'dNdLogDp6B',
       'dNdLogDp7B', 'dNdLogDp8B', 'dNdLogDp9B', 'dNdLogDp10B', 'dNdLogDp11B',
       'dNdLogDp12B', 'dNdLogDp13B', 'dNdLogDp14B', 'dNdLogDp15B', 'Source', 'Type'])

POPSsd_m_dpB = POPSsd_m.filter(['Launch', 'Theta', 'Altitude (km)', 'Theta [K]', 'Dp1B', 'Dp2B',
       'Dp3B', 'Dp4B', 'Dp5B', 'Dp6B', 'Dp7B', 'Dp8B', 'Dp9B', 'Dp10B', 'Dp11B', 'Dp12B',
       'Dp13B', 'Dp14B', 'Dp15B', 'Source', 'Type'])

POPS_SD_colnamesB = ['dNdLogDp1B','dNdLogDp2B', 'dNdLogDp3B', 'dNdLogDp4B', 'dNdLogDp5B', 'dNdLogDp6B',
       'dNdLogDp7B', 'dNdLogDp8B', 'dNdLogDp9B', 'dNdLogDp10B', 'dNdLogDp11B',
       'dNdLogDp12B', 'dNdLogDp13B', 'dNdLogDp14B', 'dNdLogDp15B']

POPS_DP_colnamesB = ['Dp1B', 'Dp2B',
       'Dp3B', 'Dp4B', 'Dp5B', 'Dp6B', 'Dp7B', 'Dp8B', 'Dp9B', 'Dp10B', 'Dp11B', 'Dp12B',
       'Dp13B', 'Dp14B', 'Dp15B']
...
#for composition B
POPSsd_test_concB = pd.melt(POPSsd_m_concB,id_vars=['Launch','Theta','Altitude (km)'], value_vars=POPS_SD_colnamesB, var_name = 'Bins', value_name = 'Concentration')
POPSsd_test_dpB = pd.melt(POPSsd_m_dpB,id_vars=['Launch','Theta', 'Altitude (km)'], value_vars=POPS_DP_colnamesB, var_name = 'Diameter', value_name = 'Dp')
POPSsd_test_concB = POPSsd_test_concB.drop(['Launch','Theta', 'Altitude (km)'], axis = 1)

POPSsd_mfB = pd.concat([POPSsd_test_dpB, POPSsd_test_concB], axis = 1)
POPSsd_mfB = POPSsd_mfB.drop(['Bins', 'Diameter'], axis=1)
POPSsd_mfB['Source'] = 'POPS_IR=1.41+0.02j'
...
conditions = [(POPSsd_mf['Launch'] == '2019-09-03'), (POPSsd_mfB['Launch'] == '2020-01-27'), (POPSsd_mfB['Launch'] == '2020-06-19'),
       (POPSsd_mfB['Launch'] == '2020-07-23'), (POPSsd_mfB['Launch'] == '2020-11-17')]   
choices = ['Volcanic', 'BB', 'Other', 'Aged BB', 'Approaching Baseline']
POPSsd_mfB['Type']=np.select(conditions, choices, default = 'Unassigned')

##vertcat model and measu size dist dataframes together
SizeDist = pd.concat([Carma_test, POPSsd_mf, POPSsd_mfB], ignore_index = True, axis=0)
...
if figures == 1:
    ...
    palette1a = {"POPS":"black",
                "CARMA":"firebrick"
                    }
    
    palette1a = {"POPS":"black",
                "CARMA":"firebrick",
                "POPS IR=1.41+0.02j": "peru"
                    }
    ...
    palette5 = seaborn.color_palette("cubehelix", 8) 
    
    hue_order_ozwv = ["2019-01-23", "2019-02-27", "2019-04-02", "2019-04-25", "2019-06-04", "2019-07-04", "2019-10-07", "2019-11-11", "2019-12-30", "2020-03-04", "2020-03-29", "2020-05-04", "2020-05-27", "2020-09-01", "2020-10-08",
                "2020-12-14", "2021-02-04", "2021-03-01", "2021-04-08", "2021-05-26", "2021-06-30", "2021-08-03", "2021-10-15",  "2019-04-30",  "2020-06-19",
                 "2020-11-17",  "2021-08-31",  "2021-11-03", "2022-01-24",  "2019-09-03", "2020-01-27","2020-07-23", "2021-01-11"]
    
    hue_order_aer = ["2019-04-30",  "2020-06-19", "2020-11-17",  "2021-08-31",  "2021-11-03", "2022-01-24", "2019-09-03", "2020-01-27",  "2020-07-23"]
    ...
    #FIGURE 5!
    SizeDist_a  = SizeDist
    SizeDist_a['Concentration_ed'] = SizeDist_a.apply(lambda x: SDed(x['Concentration'], x['Source'], x['Type'], x['Theta']), axis=1)
    
    SizeDist_a["Source"]=SizeDist_a["Source"].str.replace("_", " ", case = True)  
    seaborn.set(font_scale=1, style="ticks")
    g = seaborn.relplot(x="Dp", y="Concentration_ed", style = 'Source', style_order=(['CARMA','POPS','POPS IR=1.41+0.02j']),
                        hue = 'Source', hue_order = (['CARMA', 'POPS','POPS IR=1.41+0.02j']),  #hue = "Launch", hue_order = ([ '2019-09-03', '2020-01-27', '2020-07-23', '2020-11-17']),
                        col = 'Theta', col_order = ([410, 510]), row = 'Type', row_order = ([ 'Volcanic', 'BB', 'Aged BB', 'Approaching Baseline']), kind="line",  marker = 's', palette=palette1a, data= SizeDist_a, facet_kws={'sharey': True, 'sharex': True}) #drawstyle='steps-post', 
    ...
    g = seaborn.lineplot(x="Aconc_q2", y="Altitude (km)", data=df_AC, orient='y', sort = False, hue="Launch", hue_order = hue_order_aer,
                            linestyle='solid', linewidth=1.0, palette= palette0,  zorder = 0, legend=False)
    ...
    ax.text(17, 28.0, 'a.', size=14, color='black')
    ...
    custom_lines2 = [Line2D([0], [0], color=  'r',  lw=3),
                Line2D([0], [0], color =  'k',  lw=3)]

    ax.legend(custom_lines2, [ 'SH', 'NH'])
    ...
    #plotorder = df5.sort_values(by=['Launch','Flight','Altitude (km)'])
    #seaborn.set(style="darkgrid")
    g = seaborn.lineplot(x="AconcSTP_q2", y="Altitude (km)", data=df_AC_STP, orient='y', sort = False, hue="Launch", hue_order = hue_order_aer,
                            linestyle='solid', linewidth=1.0, palette= palette0,  zorder = 0, legend = False)
    ...
    plt.axhline(y = 13.9, color = 'k', linestyle = ':')
    plt.axhline(y = 15.75, color = 'r', linestyle = ':')
    ax.text(300, 28.0, 'b.', size=14, color='black')
    ...
    fig, ax = plt.subplots(1, 1, figsize=(8, 4))
    g = seaborn.lineplot(x="Erad_q2", y="Altitude (km)", data=df_ER, orient='y', sort = False, hue="Launch", hue_order = hue_order_aer,
                              palette= palette0, linestyle='solid', linewidth=1.0, zorder = 0, legend = False) #
    g.set(xlim=[50, 600])
    g.set(ylim=[10, 30])
    ...
    plt.axhline(y = 13.9, color = 'k', linestyle = ':')
    plt.axhline(y = 15.75, color = 'r', linestyle = ':')
    ax.text(500, 28.0, 'c.', size=14, color='black')
    ...
    #plotorder = df5.sort_values(by=['Launch','Flight','Altitude (km)'])
    #seaborn.set(style="darkgrid")
    g = seaborn.lineplot(x="Asa_q2", y="Altitude (km)", data=df_SA, orient='y', sort = False, hue="Launch", hue_order = hue_order_aer,
                              palette= palette0, linestyle='solid', linewidth=1.0, zorder = 0, legend = False) #
    ...
    plt.axhline(y = 13.9, color = 'k', linestyle = ':')
    plt.axhline(y = 15.75, color = 'r', linestyle = ':')
    ax.text(300, 28.0, 'd.', size=14, color='black')
    ...
    fig, ax = plt.subplots(1, 1, figsize=(8, 4))
    g = seaborn.lineplot(data=df_H2O, x="H2O_q2", y="Altitude (km)", orient='y', sort = False, palette= palette2, linestyle='solid', linewidth=1.0, legend = False, hue="Launch", hue_order = hue_order_ozwv, zorder = 0) #
    ...
    plt.axhline(y = 13.9, color = 'k', linestyle = ':')
    plt.axhline(y = 15.75, color = 'r', linestyle = ':')
    ax.text(200, 28.0, 'f.', size=14, color='black')
    ...
    g = seaborn.lineplot(x="O3_q2", y="Altitude (km)", data=df_O3, hue="Launch", hue_order = hue_order_ozwv, orient='y', sort = False,
                      palette= palette2, legend = False, linestyle='solid', linewidth=1.0, zorder = 0) 
    ...
    plt.axhline(y = 13.9, color = 'k', linestyle = ':')
    plt.axhline(y = 15.75, color = 'r', linestyle = ':')
    ax.text(8, 28.0, 'e.', size=14, color='black')
    #plot Boulder water IQR +/- median
    ...
    df_int_all["Source"]= df_int_all["Source"].str.replace("CARMA calc.", "CARMA - 521 nm", case = False)  
