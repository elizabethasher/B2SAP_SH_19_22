...
df_all = pd.read_csv(file, delimiter = ',', header = 20,  dtype=None, low_memory=False)
...
for x in range(0,7):
    ...
    if count == 5:
        df1 = df_all[df_all['Launch'] == '2019-04-30']
    ...
    if count == 0:
        #ax3.plot(x, y, 'k.', ms=1)
        ax4.contour(xi, yi2, zi, linewidths=1, levels=clevels, colors='k') #plot interpolated contours 
        cntr4 = ax4.contourf(xi, yi2, zi,  levels=clevels, extend = 'both', cmap="RdBu_r") #vmin = 0, vmax = 30,
        #fig.colorbar(cntr1, ax=ax1)#plot actual median altide bin etc...
        # 
        ax4.set_title('2020-06-19')
        #ax1.set_title('CO 07-19-2021')
        ax4.set_xlim(xlims)
        ax4.set_ylim(ylims)
        ax4.axhline(y=10.877, color = 'w', linestyle = '-.')
        ax4.text(900, 11, 'T1', size=12, color='white')
        ax4.text(950, 23, 'd.', size=14, color='white')
    ...
    if count == 1:
        ...
        #ax4.plot(x, y, 'k.', ms=2) 
        ax5.set_title('2020-07-23')
        ax5.set_xlim(xlims)
        ax5.set_xscale('log')
        ax5.set_ylim(ylims)
        ax5.axhline(y=9.369, color = 'w', linestyle = '--')
        ax5.axhline(y=16.88, color = 'w', linestyle = '-.')
        ax5.text(900, 9.5, 'T1', size=12, color='white')
        ax5.text(900, 17, 'T2', size=12, color='white')
        ax5.text(950, 23, 'e.', size=14, color='white')

    
    if count == 2:
        ax6.contour(xi, yi2, zi, linewidths=1, levels=clevels, colors='k') #plot interpolated contours  locator=ticker.LogLocator(), 
        cntr6 = ax6.contourf(xi, yi2, zi, levels=clevels, extend = 'both', cmap="RdBu_r") #locator=ticker.LogLocator(), 
        fig.colorbar(cntr6, ax=ax6, label = 'dNdLogDp' )
        #fig.colorbar(cntr5, ax=ax5, label = "Aerosol Concentration (# $\mathregular{cm^{-3}}$)")
        #ax5.plot(x, y, 'k.', ms=2) 
        ax6.set_title('2020-11-17')
        ax6.set_xlim(xlims)
        ax6.set_xscale('log')
        ax6.set_ylim(ylims)
        ax6.axhline(y=9.374, color = 'w', linestyle = '-.')
        ax6.text(900, 9.4, 'T1', size=12, color='white')
        ax6.text(950, 23, 'f.', size=14, color='white')
    ...
    if count == 3:
        ...
        #ax1.plot(x, y, 'k.', ms=2) 
        ax2.set_title('2019-09-03')
        ax2.set_xlim(xlims)
        ax2.set_xscale('log')
        ax2.set_ylim(ylims)
        ax2.axhline(y=10.125, color = 'w', linestyle = '-.')
        ax2.text(900, 10.2, 'T1', size=12, color='white')
        ax2.text(950, 23, 'b.', size=14, color='white')
    
    if count == 4:
        ax3.contour(xi, yi2, zi, linewidths=1, levels=clevels, colors='k') #plot interpolated contours locator=ticker.LogLocator(),
        cntr3 = ax3.contourf(xi, yi2, zi,  levels=clevels, extend = 'both', cmap="RdBu_r") #locator=ticker.LogLocator(),
        #fig.colorbar(cntr5, ax=ax5, label = 'Aerosol Concentration (#/cm3)' )
        #ax2.plot(x, y, 'k.', ms=2) 
        ax3.set_title('2020-01-27')
        ax3.set_xlim(xlims)
        ax3.set_xscale('log')
        ax3.set_ylim(ylims)
        ax3.axhline(y=12.377, color = 'w', linestyle = '--')
        ax3.axhline(y=16.126, color = 'w', linestyle = '-.')
        ax3.text(900, 12.5, 'T1', size=12, color='white')
        ax3.text(900, 16.2, 'T2', size=12, color='white')
        ax3.text(950, 23, 'c.', size=14, color='white')
    ...
    if count == 5:
        ...
        #fig.colorbar(cntr5, ax=ax5, label = 'Aerosol Concentration (#/cm3)' )
        #ax2.plot(x, y, 'k.', ms=2) 
        ax1.set_title('2019-04-30')
        ax1.set_xlim(xlims)
        ax1.set_xscale('log')
        ax1.set_ylim(ylims)
        ax1.axhline(y=13.126, color = 'w', linestyle = '-.')
        ax1.text(900, 13.2, 'T1', size=12, color='white')
        ax1.text(950, 23, 'a.', size=14, color='white')
